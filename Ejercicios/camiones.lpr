...
procedure merge(c:camiones; var a:arbol);
  procedure crearArbol(var a:arbol; total:integer;av:integer);
  begin
    if(a=nil)then begin
      new(a);
      a^.dato.num:=av;
  ...
  procedure minimo(var c:camiones;var cant:integer;var minAv:integer);
  var
...
var
  total,cant:integer;
  minAv,actualAv:integer;
begin
  a:=nil;
  minimo(c,cant,minAv);
  while(minAv<>999)do begin
    actualAv:=minAv;
    total:=0;
    while(minAv<>999)and(actualAv=minAv)do begin
      total:=total+cant;
      minimo(c,cant,minAv);
    end;
